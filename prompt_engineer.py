...
# --------------------------------------------------------------------------
# Strategy: Sequential BATCH MODE - NEW THREE-STAGE ARCHITECTURE
# --------------------------------------------------------------------------
...
def create_sequential_batch_stage2_grammar_prompt(job_list, stage1_outputs):
    ...
    Simplified instructions focus exclusively on generation without validation or justification.
    """
...
3. **PLAUSIBLE LEARNER ERRORS:** Generate common interlanguage patterns such as:
...
4. **INFLECTIONS AND DERIVATIONS PERMITTED:** You may modify verb forms, add or remove inflections, and adjust derivations to create grammatically incorrect constructions.

5. **NO LEXICAL OVERLAP:** Do not use any form of the correct answer word or its root in candidates unless testing word form distinctions.

6. **ANTI-REPETITION:** Avoid using identical candidate words across multiple questions in this batch unless required by the Assessment Focus.
...
2. **SAME WORD CLASS AND GRAMMATICAL FORM REQUIRED:** All candidates must match the exact grammatical structure of the correct answer. If the correct answer is a present tense third person singular verb, all candidates must be present tense third person singular verbs.

3. **SEMANTIC FIELD PROXIMITY:** Generate alternatives from the SAME semantic field as the correct answer. For example:
...
4. **CONTEXTUAL INAPPROPRIATENESS TYPES:**
...
5. **PHONETIC AND SEMANTIC ALTERNATIVES PERMITTED:** For higher-level vocabulary (B1+), include at least one phonetically similar alternative that matches lexical sophistication (e.g., "possess" or "position" as alternatives to "postpone").

6. **NO LEXICAL OVERLAP:** Do not use any form of the correct answer word or its root in candidates.

7. **ANTI-REPETITION:** Avoid using identical candidate words across multiple questions in this batch.
...
def create_sequential_batch_stage3_grammar_prompt(job_list, stage1_outputs, stage2_outputs):
    ...
    Tests each candidate distractor for grammatical incorrectness and selects final three.
    """
...
3. **BINARY CLASSIFICATION:** For each candidate, answer YES or NO:
...
4. **FILTER AND SELECT:** 
   ...
   - RETAIN all candidates that produce grammatically incorrect sentences (NO answers)
...
def create_sequential_batch_stage3_vocabulary_prompt(job_list, stage1_outputs, stage2_outputs):
    ...
    Tests each candidate for grammatical correctness AND semantic inappropriateness, selects final three.
    """
...
3. **FILTERING CRITERIA:**
   ...
   - RETAIN only candidates that answer YES to Question A AND NO to Question B

4. **SELECT FINAL THREE:**
